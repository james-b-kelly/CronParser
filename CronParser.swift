...
if lines.count > 0 {
    if CommandLine.argc >= 2 {
        ...
        if let timeComponents = processInputTime(time) {
            if let results = processLines(lines, time: timeComponents) {
                for result in results {
                    print(result.displayString)
                }
            }
            else {
                print("There was an error process results for this config.")
            }
        }
        else {
        ...
        print("Please pass in a time.")
    }
}
else {
    print("No input config.")
}

func processLines(_ lines: [String], time: (hour: Int, minute: Int)) -> [Result]? {
